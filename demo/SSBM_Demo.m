...
time_SSLC_mat = zeros(num_trials,num_sizes);
Jaccard_SSLC_mat = zeros(num_trials,num_sizes);
...
for j = 1:num_sizes
    ...
    for i = 1:num_trials
        ...
        Gamma = datasample(TrueCluster,1,'Replace',false); % was 5 in IJCAI
        

        % ============= Find Cluster with SSLC ============= %
        
        tic
        Cluster_SSLC = main_SSLC(A,Gamma,n1,epsilon_LCE,3,reject_LCE,60,0.6);
        time_SSLC_mat(i,j) = toc;
        Jaccard_SSLC_mat(i,j) = Jaccard_Score(TrueCluster,Cluster_SSLC)
